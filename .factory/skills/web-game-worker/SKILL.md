...
3. If the feature will use TypeScript app code, invoke `collaborative-tdd-typescript` and write failing tests first. Keep tests focused on the contract slice completed by the feature. In non-interactive exec mode, do not block waiting for an approval pause after test design; proceed directly into red-green execution and record that deviation in the handoff.
...
6. Use `agent-browser` to manually verify the fulfilled behaviors on the real browser surface. If `agent-browser` is blocked by a local daemon or socket issue, or if Playwright MCP/browser tools report a shared-profile lock, retry with isolated browser execution when available; otherwise Playwright browser tools or the Playwright e2e runtime are acceptable fallbacks. For responsive or shared-code work, verify the relevant desktop/mobile/multi-context states explicitly.
